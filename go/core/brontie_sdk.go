...
func (sdk *BrontieSDK) OptionsMap() map[string]any {
...
func (sdk *BrontieSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *BrontieSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *BrontieSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *BrontieSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *BrontieSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath(sdk.options, []any{"allow", "op"}).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *BrontieSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath(sdk.options, []any{"allow", "op"}).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("BrontieSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *BrontieSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *BrontieSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("BrontieSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func (sdk *BrontieSDK) Balance(data map[string]any) BrontieEntity {
	return NewBalanceEntityFunc(sdk, data)
}


// Voucher returns a Voucher entity bound to this client.
// Idiomatic usage: client.Voucher(nil).List(nil, nil) or
// client.Voucher(nil).Load(map[string]any{"id": ...}, nil).
func (sdk *BrontieSDK) Voucher(data map[string]any) BrontieEntity {
	return NewVoucherEntityFunc(sdk, data)
}
