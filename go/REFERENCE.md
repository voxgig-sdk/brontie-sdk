# Brontie Golang SDK Reference

Complete API reference for the Brontie Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *BrontieSDK`
...
#### `Balance(data map[string]any) BrontieEntity`

Create a new `Balance` entity instance. Pass `nil` for no initial data.

#### `Voucher(data map[string]any) BrontieEntity`

Create a new `Voucher` entity instance. Pass `nil` for no initial data.
...
| --- | --- | --- | --- |
| `alertAt` | `any` | Yes | The threshold resolved to a euro figure: `lastTopUp × alertPercent`. |
| `alertPercent` | `float64` | Yes | Percentage of the most recent top-up at which the low-balance threshold sits. |
| `balance` | `float64` | Yes |  |
| `currency` | `string` | Yes |  |
...
```go
voucher := client.Voucher(nil)
fmt.Println(voucher.GetName()) // "voucher"
```
