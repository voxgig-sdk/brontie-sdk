...
import (
	...
	omni "github.com/voxgig-sdk/brontie-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BrontieError); is {
				out := map[string]any{
					"name":    "BrontieError",
...
func omniCtx(arg any, client *sdk.BrontieSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BrontieSDK); is {
			client = live
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.BrontieError{Msg: msg}
			}
		}
	}
...
func omniErrArg(arg any) error {
	...
	return &sdk.BrontieError{Msg: msg, Code: code}
}
