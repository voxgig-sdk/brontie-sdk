...
module BrontieOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if BrontieOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = BrontieOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| BrontieOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || BrontieOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = BrontieSpec.new(spec_map) if spec_map.is_a?(::Hash)

    res_map = ctxmap['result']
    if res_map.is_a?(::Hash)
      ctx.result = BrontieResult.new(res_map)
      err_map = res_map['err']
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
         !err_map['message'].empty?
        ctx.result.err = BrontieError.new('', err_map['message'])
      end
    end

    resp_map = ctxmap['response']
    if resp_map.is_a?(::Hash)
      ctx.response = BrontieResponse.new(resp_map)
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { BrontieOmni.errify(err) }
