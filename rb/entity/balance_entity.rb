...
class BalanceEntity
  ...
  def make
    opts = @_entopts.dup
    BalanceEntity.new(@_client, opts)
  end
  ...
  #
  # @param reqmatch [BalanceLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Balance.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Balance, Hash] the loaded Balance; raises BrontieError on failure
