...
class VoucherEntity
  ...
  def make
    opts = @_entopts.dup
    VoucherEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = BrontieHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Voucher, Hash] the current Voucher data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = BrontieHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Voucher fields)
  def match_get
  ...
  #
  # @param reqdata [VoucherCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Voucher, Hash] the created Voucher; raises BrontieError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = BrontieHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
