...
The Ruby SDK for the Brontie API — an entity-oriented client using idiomatic Ruby conventions.
...
raise a `BrontieError` (a `StandardError` subclass), so wrap
...
| --- | --- | --- |
| `alertAt` | `Object` | The threshold resolved to a euro figure: `lastTopUp × alertPercent`. |
...
```
rb/
├── Brontie_sdk.rb       -- Main SDK module
...
The main module (`Brontie_sdk`) exports the SDK class
