...
class BalanceEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = BrontieSDK::test(null, null);
        $ent = $testsdk->Balance(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = balance_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "balance." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BRONTIE_TEST_BALANCE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $balance_ref01_data = null;
        if (count($balance_ref01_data_raw) > 0) {
...
function balance_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/balance/BalanceTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["balance01", "balance02", "balance03"] as $k) {
    ...
    $entid_env_raw = getenv("BRONTIE_TEST_BALANCE_ENTID");
