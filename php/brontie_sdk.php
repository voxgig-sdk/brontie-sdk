...
class BrontieSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $utility = new BrontieUtility();
        $this->_utility = $utility;

        $config = BrontieConfig::shared_config();
        ...
        $feature_opts = BrontieHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = BrontieHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!BrontieFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, BrontieFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return BrontieUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = BrontieHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = BrontieHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = BrontieHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new BrontieError($op . "_allow",
                "BrontieSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = BrontieHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new BrontieError("graphql_error",
                "BrontieSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    // Canonical facade: $client->Voucher()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->voucher()
    // resolves here too.
    public function Voucher($data = null)
    {
        require_once __DIR__ . '/entity/voucher_entity.php';
        if ($data === null) {
            if ($this->_voucher === null) {
                $this->_voucher = new VoucherEntity($this, null);
            }
            return $this->_voucher;
        }
        return new VoucherEntity($this, $data);
    }
