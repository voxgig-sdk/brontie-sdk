...
class BalanceEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new BalanceEntity($this->_client, $opts);
    }

    /**
     * @param Balance|array $args Balance data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = BrontieHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Balance|array The current Balance data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Balance fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = BrontieHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Balance fields).
     */
    ...
    /**
     * Load a single Balance.
     *
     * @param BalanceLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed BalanceLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Balance|array The loaded Balance as an assoc-array at the
     *   SDK boundary; throws BrontieError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = BrontieHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
