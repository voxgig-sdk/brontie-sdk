...
The PHP SDK for the Brontie API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Balance()` — with named operations (`load`/`create`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```php
<?php
require_once 'brontie_sdk.php';

$client = new BrontieSDK([
    "apikey" => getenv("BRONTIE_APIKEY"),
]);
```
...
```php
$client = BrontieSDK::test($testopts, $sdkopts);
```
...
| `Voucher` | `($data): VoucherEntity` | Create a Voucher entity instance. |
...
| --- | --- | --- |
| `alertAt` | `mixed` | The threshold resolved to a euro figure: `lastTopUp × alertPercent`. |
| `alertPercent` | `float` | Percentage of the most recent top-up at which the low-balance threshold sits. |
| `balance` | `float` |  |
| `currency` | `string` |  |
...
Create an instance: `$voucher = $client->Voucher();`
...
```
php/
├── brontie_sdk.php          -- Main SDK class
...
The main class (`brontie_sdk.php`) exports the SDK class
