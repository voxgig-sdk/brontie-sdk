...
class BrontieMakeError
{
    public static function call(?BrontieContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new BrontieContext([], null);
        }
        $op = $ctx->op ?? new BrontieOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new BrontieResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof BrontieError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new BrontieError('', $msg, $ctx);
        ...
        if ($err instanceof BrontieError) {
