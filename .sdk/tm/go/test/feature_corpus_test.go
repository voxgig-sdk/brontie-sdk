...
func fcClient(kase map[string]any) *sdk.BrontieSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
...
func fcCandidates(client *sdk.BrontieSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.BrontieSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
func fcRecord(client *sdk.BrontieSDK, name string) any {
