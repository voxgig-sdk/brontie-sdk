...
module BrontieUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = BrontieContext.new({}, nil)
    end
    op = ctx.op || BrontieOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || BrontieResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(BrontieError) ? err.msg : err.to_s
    ...
    sdk_err = BrontieError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(BrontieError)
