...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = BrontieOmni.make_runner(TEST_JSON_FILE, BrontieSDK.test(nil, nil))
  ...
  def test_feature_add_basic
    ...
    feature = BrontieBaseFeature.new
  ...
  def test_feature_hook_basic
    hook_client = BrontieSDK.test(nil, nil)
  ...
  def test_feature_init_basic
    init_client = BrontieSDK.test(nil, nil)
  ...
  def test_make_result_no_result
    ctx = make_test_full_ctx(@client, @utility)
    ctx.spec = BrontieSpec.new({ "step" => "start" })
    ctx.result = nil
  ...
  def test_make_spec_basic
    ...
    spec_client = BrontieSDK.test(nil, setup_opts)
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : BrontieOmni.errify(err)
    end
  ...
  def test_make_url_basic
    subject = lambda do |ctx|
      ctx.result = BrontieResult.new({}) unless ctx.result
  ...
  def test_prepare_auth_basic
    ...
    auth_client = BrontieSDK.test(nil, setup_opts)
  ...
  def auth_credential(client)
    utility = client.get_utility
    ctx = make_test_ctx(client, utility, nil)
    ctx.spec = BrontieSpec.new({ "headers" => {}, "query" => {}, "step" => "s" })
