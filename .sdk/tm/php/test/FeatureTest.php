...
class FtCtrl extends BrontieControl
{
...
class FtClient
{
    ...
    private BrontieUtility $utility;

    public function __construct(BrontieUtility $utility, array $options)
    {
    ...
    public function get_utility(): BrontieUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BrontieContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (BrontieContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BrontieConfig::make_feature($name);
    ...
    private static function buildUrl(BrontieSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BrontieError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BrontieResponse($fetched);
        }
    ...
    private function fail(BrontieContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BrontieContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
