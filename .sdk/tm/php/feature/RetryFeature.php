...
class BrontieRetryFeature extends BrontieBaseFeature
{
    ...
    public function init(BrontieContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (BrontieContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(BrontieContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(BrontieContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
