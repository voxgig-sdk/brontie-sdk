...
type Args struct {
	Entity string         `json:"entity" jsonschema:"balance | voucher"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.BrontieSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "brontie_list",
		Description: "List records from Brontie. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "brontie_load",
		Description: "Load a single record from Brontie. " +
...
func runOp(client *sdk.BrontieSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.BrontieSDK, name string) (sdk.BrontieEntity, error) {
	switch strings.ToLower(name) {
	case "balance":
		return client.Balance(nil), nil
	case "voucher":
		return client.Voucher(nil), nil
