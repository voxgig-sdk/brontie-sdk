...
[MCP](https://modelcontextprotocol.io) server exposing the Brontie SDK as
two agent tools — `brontie_list` and `brontie_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/brontie-mcp)
make build

# 2. Provide credentials via the environment
export BRONTIE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user brontie \
  -- /absolute/path/to/brontie-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./brontie-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// brontie_list: first page of records
{ "entity": "balance" }
{ "entity": "balance", "query": { } }

// brontie_load: one record by id
{ "entity": "balance", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/brontie-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user brontie \
     -- "$PWD"/dist/*/brontie-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `brontie_list` and `brontie_load` tools now appear
   in new sessions. Ask the agent to *"list balance using brontie"*
   and it calls `brontie_list` with `{"entity":"balance"}`.
...
```sh
export BRONTIE_APIKEY=sk_live_xxx            # API key
export BRONTIE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `brontie_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `brontie_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BRONTIE_APIKEY` | API key sent with every request. |
| `BRONTIE_BASE` | Optional override of the API base URL. |
...
```sh
./brontie-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"brontie_load","arguments":{"entity":"balance","query":{"id":1}}}}'
```
