...
The Lua SDK for the Brontie API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Balance()` — each with the same small set of operations (`load`, `create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
| --- | --- | --- |
| `alertAt` | `number|nil` | The threshold resolved to a euro figure: `lastTopUp × alertPercent`. |
...
Create an instance: `local voucher = client:Voucher(nil)`
...
```
lua/
├── brontie_sdk.lua    -- Main SDK module
...
The main module (`brontie_sdk`) exports the SDK constructor
