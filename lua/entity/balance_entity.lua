...
local BalanceEntity = {}
BalanceEntity.__index = BalanceEntity


function BalanceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BalanceEntity)
  self._name = "balance"
  self._client = client
...
function BalanceEntity:get_name()
  return self._name
end


function BalanceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BalanceEntity.new(self._client, opts)
end
...
function BalanceEntity:mark_deleted()
  self._deleted = true
end


function BalanceEntity:deleted()
  return true == self._deleted
end


function BalanceEntity:data_set(args)
  if args ~= nil then
...
function BalanceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BalanceEntity:match_set(args)
  if args ~= nil then
...
function BalanceEntity:match_get()
...
function BalanceEntity:stream(action, args, callopts)
...
---@param reqmatch BalanceLoadMatch
---@param ctrl? table
---@return Balance
---@return string? err
function BalanceEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function BalanceEntity:_run_op(ctx, post_done)
  local utility = self._utility
