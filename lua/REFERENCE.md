# Brontie Lua SDK Reference

Complete API reference for the Brontie Lua SDK.
...
| --- | --- | --- | --- |
| `alertAt` | `number|nil` | Yes | The threshold resolved to a euro figure: `lastTopUp × alertPercent`. |
...
```lua
local voucher = client:Voucher(nil)
```
...
```lua
local result, err = client:Voucher():create({
