...
BrontieSDK._make_feature = _make_feature


function BrontieSDK.new(options)
  local self = setmetatable({}, BrontieSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function BrontieSDK:options_map()
...
function BrontieSDK:get_utility()
  return Utility.copy(self._utility)
end


function BrontieSDK:get_root_ctx()
  return self._rootctx
end


function BrontieSDK:prepare(fetchargs)
...
function BrontieSDK:direct(fetchargs)
...
-- by passing it.
function BrontieSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function BrontieSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "BrontieSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function BrontieSDK:Balance(data)
...
-- Idiomatic facade: client:Voucher():list() / client:Voucher():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function BrontieSDK:Voucher(data)
  local EntityMod = require("entity.voucher_entity")
  if data == nil then
    if self._voucher == nil then
      self._voucher = EntityMod.new(self, nil)
    end
...
function BrontieSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = BrontieSDK.new(sdkopts)
  sdk.mode = "test"
