...
runtime for the `idempotency` feature in the TypeScript target — do
...
| Runtime template | `.sdk/tm/ts/src/feature/idempotency/` (copied here on `generate`; `FEATURE_Name`/`FEATURE_VERSION` substituted) |
...
- **Behaviour**: edit the runtime template under
  `.sdk/tm/ts/src/feature/idempotency/`, then regenerate.
