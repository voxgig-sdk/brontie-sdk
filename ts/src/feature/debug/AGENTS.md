...
runtime for the `debug` feature in the TypeScript target — do
...
| Part | Path |
| --- | --- |
| Model definition | `.sdk/model/feature/debug.aontu` (name, title, version, `config.options.active`, the `hook` map, per-language `deps`) |
| Registered in | `.sdk/model/feature/feature-index.aontu` (`@"debug.aontu"`) |
| Runtime template | `.sdk/tm/ts/src/feature/debug/` (copied here on `generate`; `FEATURE_Name`/`FEATURE_VERSION` substituted) |
...
- **Turn hooks on/off**: edit the `hook` map in
  `.sdk/model/feature/debug.aontu` (`<Stage>: active: true|false`).
...
- **Behaviour**: edit the runtime template under
  `.sdk/tm/ts/src/feature/debug/`, then regenerate.
