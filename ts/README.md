# Brontie TypeScript SDK



The TypeScript SDK for the Brontie API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Balance()` — each with a small set of operations (`load`, `create`)
...
```ts
const client = new BrontieSDK({ apikey: '...' })
...
| `tester(testopts?, sdkopts?)` | `BrontieSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `BrontieSDK.test(testopts?, sdkopts?)` | `BrontieSDK` | Create a test-mode client. |
...
| `client` | `client(): BrontieSDK` | Return the parent SDK client. |
...
| --- | --- | --- |
| `alertAt` | `number | null` | The threshold resolved to a euro figure: `lastTopUp × alertPercent`. |
| `alertPercent` | `number` | Percentage of the most recent top-up at which the low-balance threshold sits. |
| `balance` | `number` |  |
| `currency` | `string` |  |
...
Create an instance: `const voucher = client.Voucher()`
