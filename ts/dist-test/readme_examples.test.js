...
const TEST_SEED = { "entity": { "balance": { "test01": { "id": "test01" } }, "voucher": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+BrontieSDK\b/.test(code) ||
        /\bBrontieSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+BrontieSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/BrontieSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
