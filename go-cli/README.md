...
boru-driven command-line client **and** interactive REPL for the Brontie
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/brontie-cli)
make build
...
./brontie-cli load 1 balance            # {id:1} shorthand
./brontie-cli load '{id:1}' balance       # explicit match map

# 5. Override the API base URL for a single call
BRONTIE_BASE=https://api.example.com ./brontie-cli load 1 balance

# 6. No arguments -> interactive REPL
./brontie-cli
brontie> load 1 balance
brontie> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/brontie-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/brontie-cli load 1 balance
   ```
...
```sh
./brontie-cli load 1 balance          # scalar shorthand for {id:1}
./brontie-cli load '{id:1}' balance     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `brontie>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/brontie-cli`. |
...
(`./brontie-cli <expr>`) and interactively (the REPL), and expressions compose the
