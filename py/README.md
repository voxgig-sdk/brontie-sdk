...
The Python SDK for the Brontie API — an entity-oriented client following Pythonic conventions.
...
client = BrontieSDK({
    "apikey": os.environ.get("BRONTIE_APIKEY"),
})
```
...
| --- | --- | --- |
| `alertAt` | `float | None` | The threshold resolved to a euro figure: `lastTopUp × alertPercent`. |
...
```
py/
├── brontie_sdk.py         -- Main SDK module
