# Brontie Python SDK Reference

Complete API reference for the Brontie Python SDK.
...
#### `BrontieSDK.test(testopts=None, sdkopts=None)`
...
Create a new `VoucherEntity` instance. Pass `None` for no initial data.
...
| --- | --- | --- | --- |
| `alertAt` | `float | None` | Yes | The threshold resolved to a euro figure: `lastTopUp × alertPercent`. |
| `alertPercent` | `float` | Yes | Percentage of the most recent top-up at which the low-balance threshold sits. |
| `balance` | `float` | Yes |  |
| `currency` | `str` | Yes |  |
...
Create a new `VoucherEntity` instance with the same options.
