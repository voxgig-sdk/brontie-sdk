...
from brontie_sdk.brontie_types import (
    Balance,
    BalanceLoadMatch,
)
...
class BalanceEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return BalanceEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Balance:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Balance().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
